...
function Section({
  ...
  return (
    <section id={id} style={{ marginBottom: 40, scrollMarginTop: 24 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: summary ? 8 : 14 }}>
        <span style={{
      ...
      {summary && (
        <p style={{ margin: "0 0 14px", fontSize: 15, fontWeight: 600, color: "var(--text-soft)", lineHeight: 1.6, wordBreak: "keep-all" }}>
          {summary}
        </p>
      )}
  ...
  return (
    <details className="guide-detail" style={{ marginTop: 14 }}>
      <summary style={{
        ...
        borderRadius: 999, userSelect: "none", width: "fit-content",
      }}>
        <span className="gd-arrow" style={{ fontSize: 10, display: "inline-block" }}>▼</span>
        {label}
      </summary>
      ...
      <div>
        ...
        <div style={{
          ...
          <div style={{ marginTop: 10, fontSize: 14, color: "var(--text-soft)", lineHeight: 1.7 }}>
            <b>{user.name}</b> 선생님의 1인 사물함 사용 안내. 각 기능의 <b>사용법 영상</b>을 먼저 보고,
            자세한 단계는 <b>‘자세히 보기’</b>를 펼쳐 확인하세요.
          </div>
        </div>
        ...
        {/* 1. 시작하기 */}
        <Section id="intro" num="1" title="시작하기" summary="초대코드로 가입하면 본인 사물함이 즉시 열려요. 본인 자료는 본인만 봅니다.">
        ...
        <Section id="settings" num="2" title="내 설정" summary="한 번 저장하면 일정표·기록지에 자동으로 반영돼요. 지역별 기록지 서식도 여기서 골라요.">
          ...
          <Detail>
            ...
            <Step n={5} title="기록지 서식 (지역 양식)">
              ...
              현재 지원: 표준형(기본)·익산형·동탄형·남양주형·순천형·원주형·대구·파주·광명·시흥·안산형. (양식은 계속 추가돼요)
            </Step>
            <Callout kind="tip">
        ...
        <Section id="children" num="3" title="내 아동 등록" summary="한 번 등록하면 매월 클릭 한 번에 정보가 채워져요. 직접·엑셀로 등록할 수 있어요.">
          ...
          <Detail>
            ...
            <Step n={2} title="한 명씩 자세히 등록">
              <b>[내 아동] → [한 명씩 등록]</b> 에서 이름·생년월일·관리번호·메모·기본 시간대·반복 요일·회당 단가·월 본인부담금·월 목표 회기까지 모두 입력.
              서비스 종류와 담당 치료사는 <b>[내 설정]</b> 의 치료사 종류에 따라 본인으로 자동 고정돼요.
              반복 요일을 고르면 요일마다 시간대를 따로 지정할 수도 있어요.
            </Step>
            <Step n={3} title="엑셀로 한꺼번에 가져오기">
              <b>[내 아동] → [엑셀로 가져오기]</b> → 쓰던 명단이나 전자바우처 <b>‘서비스제공내역.xls’</b> 를 그대로 업로드 →
              컬럼(성명·생년월일·시간·요일·단가·본인부담금·목표)을 자동 인식해 미리보기 → <b>[이대로 저장]</b>.
              가져온 아동은 모두 <b>본인 담당</b>으로 등록됩니다. 양식이 없으면 <b>[기본 양식 다운로드]</b> 로 받아 채우세요.
            </Step>
        ...
        <Section id="bulk" num="9" title="일괄 다운로드" summary="월말 마감, 아동별 한글파일을 ZIP으로 한 번에 받아요.">
          ...
          <Detail>
            ...
            <Step n={2} title="월 선택">
              저장된 달이 드롭다운에 뜨고, 각 달에 몇 명 분이 있는지 함께 표시돼요.
            </Step>
            <Step n={3} title="아동 선택">
              그 달의 아동 목록에서 받을 사람만 체크. <b>[전체 선택]</b> 한 번이면 모두 선택돼요.
            </Step>
            <Step n={4} title="다운로드">
              <b>[선택한 N명 다운로드]</b> → 아동별 .hwpx 가 ZIP 한 파일로 묶여 받아져요.
              6회기 이상인 기록지는 자동 분할됩니다.
            </Step>

            <Callout kind="tip">
              12명 전부면 [전체 선택] 후 1번 클릭. 일부만 필요하면 그 아동만 골라서 받으세요.
