...
// 승인일자 등 날짜 칸을 '월/일'로 — 연도를 떼고 M/D(앞 0 제거, 슬래시) 형태로 통일.
// "2026-06-07"→"6/7", "2026.6.2"→"6/2", "2026년 6월 7일"→"6/7", "06-07"→"6/7".
// 월·일을 못 뽑으면(일 숫자만 "7", 빈값) 원본 그대로 둔다.
export function monthDayOnly(s: string): string {
  const t = String(s ?? "").trim();
  // 4자리 연도 + 월 + 일  → 월/일
  let m = t.match(/\d{4}\D+(\d{1,2})\D+(\d{1,2})/);
  if (m) return `${+m[1]}/${+m[2]}`;
  // 연도 없이 두 숫자(월·일)  → "M/D" 로 정규화 ("06-07"·"6.7"·"6월 7일"·"6/14")
  m = t.match(/^\D*(\d{1,2})\D+(\d{1,2})\D*$/);
  if (m) return `${+m[1]}/${+m[2]}`;
  return t;
}
