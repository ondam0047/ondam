...
export async function llmSuggestRoles(
  ...
  const formTypeLine =
    opts.formType === "schedule"
      ? "[양식 종류] 이 양식은 '일정표'다. 회기별 결과/시간 칸이 아니라 관리번호·단가·횟수·총금액·본인부담금·주기·제공일 같은 라벨 칸과 대상자·기관·작성일자 정보를 채운다.\n\n"
      : opts.formType === "record"
        ? "[양식 종류] 이 양식은 '기록지'다. 회기별 날짜·시간·결과와 대상자 정보를 채운다. (일정표가 같이 붙은 통합 양식이면 일정표 영역의 라벨 칸은 시스템이 따로 채우니 손대지 말 것.)\n\n"
        : "";
  ...
  const system =
    ...
    "- ⚠ 확인·서명·날인 칸(예: '이용자(확인)', '보호자 확인', '서명', '날인', '(인)', '확인란', 그리고 '담당재활사'·'담당 치료사' 행의 회기별 칸 등 사람이 직접 사인·도장하는 자리)에는 어떤 역할도 주지 않는다. 특히 이런 칸에 치료사이름·비고·결과를 절대 부여하지 말 것 — 서명란은 빈칸으로 출력되어야 한다.\n" +
    "- ⚠ 월 달력 격자(요일 머리행 아래 날짜 숫자·회기 시간이 들어가는 칸들)는 시스템이 따로 채운다. 달력 표의 칸에는 어떤 역할도 주지 말 것(날짜·결과로 오인 금지).\n" +
    ...
    "- 단일(scalar) 역할은 라벨 바로 옆/아래의 '첫 값칸' 하나에만 부여한다. 같은 열 아래로 빈 행이 여러 개 이어져도(여벌 행) 반복 부여하지 말 것 — 회기반복(row) 역할만 회기 수만큼 반복한다. 같은 값이 문서의 서로 다른 구역 라벨에 각각 필요할 때(예: 기관명이 머리글과 하단 정보란)만 구역마다 한 칸씩 준다.";
