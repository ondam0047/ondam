  ...
  useFrame(() => {
    ...
    for (const b of bases.current) {
      ...
      if (b.name === "tongue" && b.baseVerts) {
        ...
        // + 전역 rest 배치(TONGUE_PLACEMENT) — 음소산출의 "혀 맞춤" 기본값과 동일하게 맞춰
        // 세 탭의 혀 위치를 일치시킨다(휴지 포함 모든 자세 공통 보정).
        const adv = TONGUE_PLACEMENT.fwd + (eff["tongue_advance"] ?? 0) * TONGUE_ADVANCE_X;
        const rise = TONGUE_PLACEMENT.up + (eff["tongue_raise"] ?? 0) * TONGUE_RAISE_Y;
