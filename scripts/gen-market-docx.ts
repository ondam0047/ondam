...
children.push(spacer());
children.push(p("일정표·제공기록지를 ‘쓰는 사람’ = 바로일지 직접 사용자", { bold: true }));
children.push(p("· 발달바우처 필수 제출 서류에 ‘계약서·월별 계획서·일정표 및 제공기록지·만족도’ 포함, 지자체 점검 때 ‘일정표와 기록지가 맞는지’ 대조 → 모든 제공인력이 매월 작성(필수). 〔효과성연구 FGI, A〕"));
children.push(p("· 회기별 제공기록지는 제공인력 본인이 작성(서명 포함). ⇒ 작성 모수 ≈ 활동 제공인력 전체 약 1.2만~1.3만명(개인 단위). 센터 일부 행정 집중 → 도구 사용 단위는 개인(~1.3만)~기관(~2,160) 사이."));
children.push(p("· FGI 원문: ‘개인 사업자(1인)는 점검 자료 불일치 多’, ‘온라인 일정표 스케줄 관리·서류 전산화로 행정부담 경감 필요’ → 바로일지의 정확한 페인·핵심 타깃(1인·개인사업자).", { italic: true, color: "667085" }));
children.push(p("⇒ 일정표·제공기록지는 ‘선택’이 아니라 모든 활동 제공인력의 ‘의무’ → 직접 사용자 상한 ≈ 1.2만~1.3만명, 그중 1인·개인사업자·프리랜서가 1순위 전환 대상.", { bold: true }));
children.push(spacer());
